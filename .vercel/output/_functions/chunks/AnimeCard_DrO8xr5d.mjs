import { c as createComponent, a as createAstro, m as maybeRenderHead, b as addAttribute, e as renderComponent, d as renderTemplate, g as renderTransition } from './astro/server_CZVEfshQ.mjs';
import 'kleur/colors';
...
const $$AnimeCard = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<article${addAttribute(`w-full h-full relative ${className}`, "class")}${addAttribute(`anime-card-${anime.id}`, "id")}> ${session ? renderTemplate`${renderComponent($$result, "AnimeListButton", AnimeListButton, { "client:visible": true, "session": session, "animeId": anime.id, "showStatus": showStatus, "className": `absolute z-50 ${!showStatus ? "top-2 right-2" : "top-0 left-0 w-full p-2"}`, "client:component-hydration": "visible", "client:component-path": "@/components/AnimeListButton.vue", "client:component-export": "default" })}` : null} <a${addAttribute(`animes/${anime.id}`, "href")} data-astro-prefetch class="overflow-hidden relative group cursor-pointer"> <div class="relative w-full h-full rounded-xl overflow-hidden bg-zinc-700 animate-pulse"> <img${addAttribute(anime.image_src, "src")}${addAttribute(anime.title, "alt")}${addAttribute(1920, "width")}${addAttribute(762, "height")} loading="lazy" class="w-full h-full object-cover rounded-xl opacity-0 transition-opacity duration-500" onload="this.style.opacity='1'; this.parentElement.classList.remove('animate-pulse', 'bg-zinc-700')"${addAttribute(renderTransition($$result, "73wb3hb4", "", `anime-image-${anime.id}`), "data-astro-transition-scope")}> </div> <div class="hidden group-hover:flex transition-all duration-1000 absolute top-0 z-3 left-0 w-full h-full background-blur-sm p-8 flex-col rounded-xl"> <div class="flex flex-col flex-1"> <h5 class="text-2xl text-white font-bold mb-1"> ${anime.title} </h5> <span class="text-sm font-semibold mb-3 text-yellow-400">
